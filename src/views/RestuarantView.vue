<template>
  <div class="about pa-3">
    ...
    >
      ...
      >
        <v-text-field
          v-model="updateFormData.restaurant_name"
          label="Restaurant Name"
          :rules="nameRules"
          required
        ></v-text-field>
...
const saveRestaurantData = async () => {
  ...
  try {
    let fd = new FormData();
    fd.append("restuarant_imageFile", formData.value.restuarant_imageFile);
    fd.append("restaurant_name", formData.value.restaurant_name);
    fd.append("restaurant_location", formData.value.restaurant_location);
    fd.append("phone_number", formData.value.phone_number);
    fd.append("place_id", formData.value.placeId);
    fd.append("restaurant_ratings", formData.value.restaurant_ratings);

    for (let [key, value] of fd.entries()) {
      console.log(key, value);
    }
    await saveRestaurant(fd);
...
const updateResturantData = async () => {
  ...
  try {
    isLoadingRequest.value = false;
    await updateRestuarantHttpRequest(updateFormData.value);
    fetchRestaurantsData();
  } catch (error) {
    errorHandler.handle(error);
  } finally {
...
const editRestuarantDetails = (data) => {
  // selectedDataId.value = id;
  updateFormData.value.restaurant_name = data.name;
  updateFormData.value.restaurant_location = data.location;
  updateFormData.value.phone_number = data.phone;
  updateFormData.value.place_id = data.placeId;
  updateFormData.value.restaurant_ratings = data.rating;
    ...
    (place, status) => {
      if (status === window.google.maps.places.PlacesServiceStatus.OK) {
        if (updateFormData.value.id) {
          ...
          updateFormData.value.restaurant_location = place.formatted_address;
          updateFormData.value.restaurant_name = place ? place.name : "";
          updateFormData.value.place_id = place ? place.place_id : "";
          updateFormData.value.phone_number = place
            ? place.formatted_phone_number
            : "";
          updateFormData.value.restaurant_ratings = place ? place.rating : "";
        } else {
          ...
          formData.value.restaurant_ratings = place ? place.rating : "";
        }
      }
    }
  );
