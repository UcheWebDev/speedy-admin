<template>
  <v-container class="pa-md-12 pa-6" fluid>
    <div class="d-flex align-center justify-space-between mb-3">
      <div>
        <div class="mb-1 text-grey">Pharmacy Items Page</div>
        <h3 class="text-h5">Manage Items</h3>
      </div>
    </div>
...
const formData = ref({
  ...
  supermarket_id: supermarket_id.value,
  image_name: null,
  image_File: null,
});
...
const submitForm = async () => {
  ...
  try {
    ...
    await axios.post(
      `/admin/pharmacies/${formData.value.pharmacy_id}/items/${formData.value.id}`,
      fd
    );
...
const confirmEvent = async () => {
  ...
  try {
    await axios.delete(
      `/admin/pharmacies/${currentId.value}/items/${toDeleteItemId.value}`
    );
    snackbar.message = "Updated successfully!";
    snackbar.show = true;
    fetchItems();
